...
case $test_mode in
    ...
    simulated)
        ...
        online2-wav-nnet3-latgen-faster --verbose=1 --max-active=4000 --beam=12.0 --acoustic-scale=0.0769 --lattice-beam=6.0 \
            --word-symbol-table=$ac_model/graph/words.txt \
	    $ac_model/final.mdl $ac_model/graph/HCLG.fst \
            "ark:echo utterance-id1 utterance-id1|" scp:$decode_dir/input.scp \
            ark,t:$ac_model/lda.mat;;
